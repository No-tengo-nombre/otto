...
otto_status_t test_creation_new(void) {
  ...
  OTTO_CALL(otto_buffer_new(sizeof(uint32_t), &vec), "Failed creating buffer");
...
otto_status_t test_creation_zero(void) {
  ...
  OTTO_CALL(otto_buffer_zero(len, sizeof(uint32_t), &vec),
...
otto_status_t test_creation_with_capacity(void) {
  ...
  log_debug("Creating buffer");
  OTTO_CALL(otto_buffer_with_capacity(6, sizeof(uint32_t), &vec),
            "Failed creating buffer");
...
otto_status_t test_creation_from_array(void) {
  ...
  log_info("Creating buffer");
  OTTO_CALL(otto_buffer_from_array(data, len, sizeof(uint32_t), &vec),
            "Failed creating buffer");
...
otto_status_t test_methods_get(void) {
  ...
  uint32_t val;
  for (uint32_t i = 0; i < len; i++) {
    OTTO_CALL(otto_buffer_get(&vec, i, &val), "Failed getting vec[%d]", i);
    OTTO_ASSERT_EQI(val, data[i]);
  }

  log_debug("Checking border scenarios");
  if (otto_buffer_get(&vec, len, &val).status != OTTO_FAILURE) {
...
otto_status_t test_methods_set(void) {
  ...
  uint32_t new0 = 20;
  uint32_t new1 = 1881;
  OTTO_CALL(otto_buffer_set(&vec, i0, &new0), "Failed ");
  OTTO_CALL(otto_buffer_set(&vec, i1, &new1), "Failed ");
  ...
  log_debug("Checking border scenarios");
  if (otto_buffer_set(&vec, len - 1, NULL).status != OTTO_FAILURE) {
...
otto_status_t test_modification_push1(void) {
  ...
  uint32_t val = 20;
  OTTO_CALL(otto_buffer_push(&vec, &val), "Failed pushing element");
  ...
  if (otto_buffer_push(&vec, NULL).status != OTTO_FAILURE) {
  ...
  OTTO_CALL(otto_buffer_get(&vec, 8, &contained_val), "Failed getting vec[8]");
...
otto_status_t test_modification_push2(void) {
  ...
  log_debug("Creating buffer");
  OTTO_CALL(otto_buffer_with_capacity(capacity, sizeof(uint32_t), &vec),
            "Failed creating buffer");
  ...
  log_debug("Checking that the push happened fine");
  uint32_t contained_val;
  OTTO_CALL(otto_buffer_get(&vec, 0, &contained_val), "Failed getting vec[0]");
  OTTO_ASSERT_EQI(contained_val, val);
  return OTTO_STATUS_SUCCESS;
}
...
otto_status_t test_modification_extend_array(void) {
  ...
  size_t values_len = 3;
  OTTO_CALL(otto_buffer_extend_array(&vec, &values, values_len),
  ...
  log_debug("Checking border scenario");
  if (otto_buffer_extend_array(&vec, NULL, 10010).status != OTTO_FAILURE) {
  ...
  uint32_t contained_val;
  for (uint32_t i = 0; i < len; i++) {
    OTTO_CALL(otto_buffer_get(&vec, i, &contained_val),
              "Failed getting vec[%d]", i);
    OTTO_ASSERT_EQI(contained_val, data[i]);
  }
  for (uint32_t i = 0; i < values_len; i++) {
    OTTO_CALL(otto_buffer_get(&vec, i + len, &contained_val),
